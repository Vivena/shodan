...
int tfs_mkdir(const char *path){
  ...
  int i, j, temp, type, partition, pemplacement, first_free_block, first_free_file, exists, index, index_entry;
  error e;
  ...
  case 1: // Si un disk : au boulot !
    ...
    while (directories[0]){
      ...
      index_entry = (index_entry % FILE_TABLE_OFFSET)*FILE_TABLE_BLOCK_SIZE;
	...
	if (!exists){ // Si n'existe pas, on créé un nouveau répertoire
	  ...
	  //index = partition+1+(current_dir/FILE_TABLE_OFFSET);
	  //read_block(id,block_entry,index);
	  entry_dir->size = TFS_DIRECTORIES_SIZE;
	  entry_dir->type = 0;
