...
int tfs_mkdir(const char *path){
    ...
    switch (type) {
        ...
        case 1: // Si un disk : au boulot !
            ...
            char* basename = basename(path_copy);
            DIR* dir = opendir(basename,id,pemplacement);
            int fd = dir->__dd_fd;
            file_descriptor fd_contain = fdtable[fd];
            
            int inode_dir = fd_contain.ninode;
            index = pemplacement+1+(inode_dir/FILE_TABLE_OFFSET);
            read_block(id,block_entry,index);
            index_entry = (inode_dir % FILE_TABLE_OFFSET)*FILE_TABLE_BLOCK_SIZE;
            memcpy(&temp,(block_entry->octets) + index_entry + (3*sizeof(uint32_t)),sizeof(uint32_t));
            index = uitoi(temp);
            read_block(id,block_navigation,index);
            
            if (is_in_directory(id, &inode_dir, pemplacement, dirname)){
